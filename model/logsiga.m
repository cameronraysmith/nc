function out1 = logsiga(varargin)
% http://goo.gl/hyLww
...
% its net output.
%	
% A = logsig(N) takes an SxQ matrix of S N-element net input column
...
%
% logsig('da_dn',N,A) returns the derivative of layer outputs A with
...
% $Revision: 1.1.6.11 $  $Date: 2011/05/09 01:02:47 $
...
function range = active_input_range, range = [-3.5 4.5]; end
...
function d = da_dn(n,a,param)
    ...
    z=10;
    s=1/2;
    d = z*exp(z*(s+n))./((exp(s*z)+exp(z*n)).^2);
    i = find(d==0);
    d(d<1e-5 & d>-1e-5) = 1.1e-5;
    i = find(~isfinite(d));
    d(i) = 5;
    %d = a.*(1-a);
end
